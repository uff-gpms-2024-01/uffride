<template>
	<q-card class="col" flat bordered>
		...
		<q-card-section class="q-pt-none q-mt-sm">
			...
			<div class="text">
				Saída: Data
				<span class="text-bold">{{ ride.date }}</span> |
		...
		<q-card-actions>
			...
			>
				<span v-if="!isDriver">Cancelar reserva</span>
				<span v-else>Cancelar carona</span>
			</q-btn>
			<q-dialog v-model="dialogReserver">
				<q-card >
					<q-card-section>
						<div class="text-h6">
							<span v-if="!isDriver(1)"
								>Deseja cancelar essa reservar?</span
							>
							<span v-else
								>Deseja cancelar essa carona?</span
							>
						</div>
					</q-card-section>
