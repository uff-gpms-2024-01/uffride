<template>
  <q-page class="flex flex-center column">
    ...
    <q-form @submit="onSubmit" class="q-gutter-md full-width q-mt-sm">
          ...
          <q-dialog v-model="stepNext">
            <q-card>
              <q-card-section class="row items-center q-pb-none">
                <div class="text-h6">Para quantos passageiros?</div>
                <q-space />
    ...
    <q-btn color="primary" label="Primary" @click="searchLocate" />
...
const optionsFn = (date) => {
  ...
  const isSameYear = selectedDate.getFullYear() === currentDate.getFullYear();
  const isSameMonth = selectedDate.getMonth() === currentDate.getMonth();

  if (isSameYear && isSameMonth) {
    return selectedDate.getDate() >= currentDate.getDate();
  } else if (isSameYear && selectedDate.getMonth() > currentDate.getMonth()) {
    return true;
  } else {
    return false;
  }
...
const rideSearch = () => {
  ...
  rideRequestStore.value.passengersNumber = passengersNumber;
...
const searchLocate = async () => {
  try {
    ...
    const response = await axios.get(`https://nominatim.openstreetmap.org/search?q=${wordSearch}&limit=2&format=json`);
